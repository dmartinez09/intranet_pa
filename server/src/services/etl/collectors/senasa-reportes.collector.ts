...
export class SenasaReportesCollector extends BaseCollector {
  ...
  async run(_ctx: CollectorContext, maps: CatalogMaps): Promise<CollectorResult> {
    ...
    try {
      ...
      console.warn('[SENASA_REPORTES] fallback por fallo:', (err as Error).message);
      snapshots.push(scoreSnapshot({
        documentTitle: 'SENASA — Reportes y registros (referencia)',
        documentUrl: URL, documentType: 'html',
        periodLabel: String(new Date().getFullYear()),
        businessNote: 'Snapshot fallback. Portal SENASA temporalmente no accesible (HTTP 502/timeout).',
      }));
