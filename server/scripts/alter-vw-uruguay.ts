// ============================================================
// ALTER vw_ventas_diarias_PointAndina_Uruguay
// - Elimina columna obsoleta GRUPO (Origen_Producto)
// - Elimina columna AGROINDUSTRIA_DISTRIBUCION (Tipo_de_Cliente)
// - Agrega columna GRUPO con Grupo_Cliente (AGROINDUSTRIAS, DIST. COSTA, etc.)
// ============================================================
...
async function main() {
  ...
  const sample = await pool.request().query(`
    SELECT TOP 3 Fecha, CLIENTE, GRUPO, FOCO
    FROM dbo.vw_ventas_diarias_PointAndina_Uruguay
    ORDER BY Fecha DESC
  `);
  console.table(sample.recordset);

  console.log('\n--- Distinct GRUPO en la vista ---');
  const grupos = await pool.request().query(`
    SELECT GRUPO, COUNT(*) AS filas
    FROM dbo.vw_ventas_diarias_PointAndina_Uruguay
    GROUP BY GRUPO
    ORDER BY filas DESC
  `);
